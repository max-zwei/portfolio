...
- With every run, check if the library was updated and compare with the version history of src/components.
...
### Takeaways before the next plan

- Translate the full Figma library before writing markup. A component set is a component in src/components. The idea would be that the full design system of Figma should already be in the repo as reusable components, tokens etc. So during translation from Figma to Code, the agents only have to match the definitions in Figma to definitions in Code.
- Write down improvements for figma-implement.md (which I would probably rename to clarify that it is specifcially for the tech stack Figma -> HTML/CSS/Astro/Decap), CLAUDE.md and design-system.d
- Include in the revised figma-implement.md the plan mode and the idea of an orchestrator. The plan mode analyses the Figma design, checks for feasibility and open questions towards Max as the designer. The orchestrator receives the plan and sends out sub-agents based on the required skills. Use this idea to verify the design for implementation, use tokens efficientyl, have a hierarchy for the "agent workforce" and reducing the incredibly high cache read.
