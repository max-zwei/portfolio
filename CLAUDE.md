...
design/components.json   Figma component-set manifest. Read it before porting a set twice.
...
docs/figma-handoff.md    The Handoff frame template, with /home worked through — plus cms (fields + editor), design-system, resume, aeo.
```
...
**Read [behind-the-scenes/skills/figma-to-astro.md](behind-the-scenes/skills/figma-to-astro.md)
...
The first move is the page's archetype — bespoke, collection index, collection
detail, or prose document — and the first read is that page's `Handoff` frame;
see [docs/figma-handoff.md](docs/figma-handoff.md). No Handoff frame means
asking its questions before building, not inferring the answers.
...
A Figma frame showing a field the schema lacks is this same three-file
change; a schema field no frame shows is either dead or the design is
incomplete — that's a question for Max, not a deletion.
